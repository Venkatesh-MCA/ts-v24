...
@Component({
  selector: 'app-ppthints',
  templateUrl: './ppthints.component.html',
  styleUrls: ['./ppthints.component.css']
})
export class PpthintsComponent {
  selectpapercode:any;
  selectedsubject:any;
  qpaper:any;
  qdata:any;
  constructor(private _papersService: PapersService, private route: ActivatedRoute) {
    ...
    this._papersService.getpaper(this.selectpapercode).subscribe(paperdata => {
      ...
      for (var q = 0; q < this.qpaper.length; q++) {
        ...
        this.qpaper[q].question_master_hint = this.qpaper[q].question_master_hint.replace(/uploads/g, imagepath);
